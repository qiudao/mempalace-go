...
func main() {
	...
	mode := flag.String("mode", "raw", "Search mode: raw, hybrid, vector, fused, or smart")
	flag.Parse()
	...
	if *mode == "vector" || *mode == "fused" || *mode == "smart" {
	...
	for i, q := range questions {
		...
		if (*mode == "vector" || *mode == "fused" || *mode == "smart") && emb != nil {
		...
		if *mode == "smart" && emb != nil {
			...
			smartResults, err := search.SmartSearch(s, q.QuestionText, queryVec, 10, store.Query{})
			if err != nil {
				fmt.Fprintf(os.Stderr, "smart search error for %s: %v\n", q.QuestionID, err)
				s.Close()
				os.RemoveAll(dir)
				continue
			}
